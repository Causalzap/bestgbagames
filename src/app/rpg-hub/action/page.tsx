...
type Game = {
  ...
  tags: string[];
  platforms?: string[];
  // 你的错误信息里出现了 release.developers
  release: {
    developers: string[];
  };
  // 如果 JSON 里有就保留；没有可以删除或改成可选
  gameDetailUrl?: string;
  whyStillWorthPlaying?: string;
};

// 如果需要从 JSON 推断强类型，直接断言为 Game[]
const games: Game[] = topGames as Game[];

export default function ActionRPGs() {
  // 过滤出 Action RPG
  const actionRpgGames: Game[] = games.filter((game) => game.genre === 'Action RPG');
  ...
  // 排序函数（保持返回新数组，避免原地排序）
  const sortedGames: Game[] = [...actionRpgGames].sort((a, b) => {
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6 pb-12">
        ...
        <div className="mb-8">
          <nav className="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <span className="text-gray-900 font-medium">Action RPGs</span>
          </nav>
        </div>

        {/* 标题 */}
        <div className="bg-gradient-to-r from-blue-800 to-purple-800 rounded-xl p-8 mb-10 text-center text-white">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Action RPGs</h1>
          <p className="text-lg text-blue-100 max-w-2xl mx-auto">
            Experience real-time combat RPGs with emphasis on action and reflexes for Game Boy Advance
          </p>
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <CategoryCard
              title="Tactical RPGs"
              description="Strategy-focused RPGs with grid-based combat and deep tactical gameplay"
              path="/rpg-hub/tactical"
              games={['Advance Wars', 'Shining Force', 'Tactics Ogre']}
            />
            <CategoryCard
              title="Adventure RPGs"
              description="Exploration-focused with light RPG elements and puzzle-solving"
              path="/rpg-hub/adventure"
              games={['Zelda: Minish Cap', "Pokémon Mystery Dungeon", 'Boktai']}
            />
          </div>
        </div>
      </div>
