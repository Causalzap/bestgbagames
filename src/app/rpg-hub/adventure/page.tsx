...
// —— 1) 明确定义数据类型 ——
// 按你 JSON 的结构：release.developers 是 string[]
type Game = {
  id: number;
  rank: number;
  title: string;
...
export default function AdventureRPGs() {
  // 过滤出 Adventure RPG
  const adventureRpgGames: Game[] = games.filter((game) => game.genre === 'Adventure RPG');
  ...
  // 排序（返回新数组）
  const sortedGames: Game[] = [...adventureRpgGames].sort((a, b) => {
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6 pb-12">
        ...
        <div className="mb-8">
          <nav className="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <Link href="/rpg-hub" className="hover:text-purple-600 transition-colors">RPG Hub</Link>
            <span>/</span>
            <span className="text-gray-900 font-medium">Adventure RPGs</span>
          </nav>
        </div>

        {/* 标题 */}
        <div className="bg-gradient-to-r from-blue-800 to-purple-800 rounded-xl p-8 mb-10 text-center text-white">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Adventure RPGs</h1>
          <p className="text-lg text-blue-100 max-w-2xl mx-auto">
            Explore worlds with light RPG elements, puzzle-solving and immersive storytelling for Game Boy Advance
          </p>
        </div>
