...
export default function TurnBasedRPGs() {
  // 过滤出 Turn-Based RPG
  const turnBasedGames: Game[] = games.filter((game) => game.genre === 'Turn-Based RPG');
  ...
  // 排序函数（返回新数组）
  const sortedGames: Game[] = [...turnBasedGames].sort((a, b) => {
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6 pb-12">
        ...
        <div className="mb-8">
          <nav className="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <span className="text-gray-900 font-medium">Turn-Based RPGs</span>
          </nav>
        </div>

        {/* 页面标题和描述 */}
        <div className="bg-gradient-to-r from-blue-800 to-purple-800 rounded-xl p-8 mb-10 text-center text-white">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Turn-Based RPGs</h1>
          <p className="text-lg text-blue-100 max-w-2xl mx-auto">
            Discover the best strategic JRPGs with deep combat systems and immersive stories for Game Boy Advance
          </p>
        </div>
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <CategoryCard
              ...
              games={['Fire Emblem', 'Final Fantasy Tactics Advance', 'Advance Wars']}
            />
            <CategoryCard
...
function GameCard({ game, onPlay }: { game: Game; onPlay: () => void }) {
  const developer =
    game.release?.developers?.length ? game.release.developers.join(', ') : 'Unknown';
